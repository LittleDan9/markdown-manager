...
function normalizeIconData(iconObj, packageWidth = 24, packageHeight = 24) {
  if (!iconObj) {
    return null;
  }
  ...
  if (typeof iconObj.body === 'string') {
    const width = iconObj.width || packageWidth;
    const height = iconObj.height || packageHeight;
    const left = iconObj.left ?? 0;
    const top = iconObj.top ?? 0;
    const vbW = iconObj.width || packageWidth;
    const vbH = iconObj.height || packageHeight;
...
class IconPackManager {
  ...
  async loadIconifyPack(config) {
    try {
      ...
      const packageWidth = data.icons?.width || data.width || 24;
      const packageHeight = data.icons?.height || data.height || 24;
      ...
      Object.entries(iconsData).forEach(([key, iconObj]) => {
        try {
          const norm = normalizeIconData(iconObj, packageWidth, packageHeight);
          if (norm) {
