...
export default function GitHubModal({ show, onHide }) {
  ...
  }, [accounts, accountsLoading]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleAccountsChange = useCallback((_updatedAccounts) => {
  ...
  const handleRepositoryBrowse = (repository) => {
    ...
    const returnCallback = () => {
      // Small delay to ensure proper modal transition and avoid layering issues
      setTimeout(() => {
      ...
      }, 300); // Increased delay to ensure file modal is fully closed
    };
