...
class AuthUI {
  ...
  bindEvents() {
    ...
    // Show login modal on loginBtn click
    const loginBtn = document.getElementById("loginBtn");
    console.log(loginBtn, "Login button element");
    if (loginBtn) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.mfa_required) {
          MFAHandler.showMFAModal({
            email: email,
            password: password,
            temp_token: data.temp_token, // Pass any required session data
          });
        } else {
