...
class MFAManager {
    ...
    try {
      ...
      if (response.ok) {
        ...
        NotificationManager.showError(
          error.detail || "Failed to start MFA setup",
        );
      }
    } catch (error) {
  ...
  showSetupModal() {
    ...
    setTimeout(() => {
      ...
      if (!modalElement) {
        ...
        NotificationManager.showError(
          "Failed to create MFA setup modal. Please try again.",
        );
        return;
      }
      ...
      try {
        ...
        NotificationManager.showError(
          "Failed to initialize MFA setup modal. Please try again.",
        );
  ...
  setupModalEventListeners() {
    ...
    document
      .getElementById("mfaNextToVerify")
    ...
    document
      .getElementById("mfaBackToVerify")
      ?.addEventListener("click", () => {
        this.goToAccordionStep(2);
      });
  ...
  async verifySetup() {
    ...
    SpinnerManager.show("mfaVerifyBtn", { message: "Verifying MFA..." });
    ...
    try {
      ...
      SpinnerManager.hide("mfaVerifyBtn");
    }
  }
  ...
  async enableMFAWithPassword() {
    if (!this.verifiedTotpCode) {
      NotificationManager.showError(
        "No verified TOTP code available. Please start over.",
      );
      return;
    }
    ...
    SpinnerManager.show("mfaEnableFinalBtn", { message: "Enabling MFA..." });
    this.hideError(errorEl);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.backup_codes) {
          ...
          const backupCodesContainer =
            document.getElementById("mfaBackupCodes");
          if (backupCodesContainer) {
            backupCodesContainer.innerHTML = result.backup_codes
              .map(
                (code) =>
      ...
      SpinnerManager.hide("mfaEnableFinalBtn");
    }
  }
  ...
  completeSetup() {
    ...
    if (modal && mfaModalElement) {
      ...
      mfaModalElement.addEventListener(
        "hidden.bs.modal",
        () => {
          ...
          if (
            this.authManager &&
            typeof this.authManager.checkAuthStatus === "function"
          ) {
            this.authManager
              .checkAuthStatus()
              .then(() => {
  ...
  hideDisableMFAForm() {
    ...
    if (submitBtn) {
      ...
      submitBtn.innerHTML =
        '<span class="spinner-border spinner-border-sm me-2 d-none" id="disableMFASpinner"></span>Disable MFA';
    }
  ...
  async disableMFA() {
    ...
    if (!password || !code) {
      this.showDisableMFAError(
        "Please enter both password and authentication code",
      );
      return;
    }
  ...
  async toggleBackupCodesSection() {
    ...
    if (isShown) {
      ...
      const collapse =
        bootstrap.Collapse.getOrCreateInstance(backupCodesSection);
      collapse.hide();
    } else {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const error = await response.json();
          NotificationManager.showError(
            error.detail || "Failed to retrieve backup codes",
          );
        }
      } catch (error) {
  ...
  updateBackupCodesButton(isVisible) {
    ...
    if (isVisible) {
      toggleBtn.innerHTML =
        '<i class="bi bi-eye-slash me-2"></i>Hide Backup Codes';
    } else {
  ...
  populateBackupCodesSection(backupCodes) {
    ...
    if (container) {
      container.innerHTML = backupCodes
        .map(
          (code) =>
