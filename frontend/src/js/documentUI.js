...
export class DocumentUIManager {
  ...
  async showSaveAsModal(isRename = false, documentToRename = null) {
    try {
      ...
      const displayName =
        currentName === "Untitled Document" || !currentName.trim()
          ? ""
          : currentName;
          ...
          input.addEventListener("input", (e) => {
            ...
            if (!value || value === "Untitled Document") {
              ...
              helpText.textContent =
                'Please enter a valid document name (cannot be empty or "Untitled Document")';
              helpText.className = "form-text text-danger";
            } else {
              actionButton.disabled = false;
              helpText.textContent =
                'Enter a name for your document (cannot be "Untitled Document")';
            ...
            if (categoryName) {
              if (documentManager.addCategory(categoryName)) {
                ...
                NotificationManager.showError(
                  "Category already exists or invalid name",
                );
              }
            } else {
      ...
      NotificationManager.showError(
        "Error opening Save dialog: " + error.message,
      );
    }
  }
  ...
  async performSaveAs() {
    ...
    if (name === "Untitled Document") {
      NotificationManager.showError(
        "Cannot save with the default document name. Please choose a different name.",
      );
      nameInput?.focus();
      nameInput?.select();
      return;
  ...
  async performRename(originalDoc) {
    try {
      ...
      if (
        newName === originalDoc.name &&
        newCategory === originalDoc.category
      ) {
        this.closeModal();
        return;
      }
      ...
      NotificationManager.showError(
        "Failed to rename document: " + error.message,
      );
    }
  }
  ...
  exportMarkdown() {
    try {
      ...
      NotificationManager.showError(
        "Error exporting markdown: " + error.message,
      );
    }
  }
  ...
  saveDocumentTitle() {
    ...
    if (newTitle === "Untitled Document") {
      NotificationManager.showError(
        'Cannot use "Untitled Document" as document name',
      );
      this.cancelTitleEdit();
      return;
    }
    ...
    if (newTitle !== oldName) {
      ...
      if (documentManager.currentDocument.id) {
        try {
          ...
          NotificationManager.showError(
            "Error renaming document: " + error.message,
          );
        }
      } else {
        ...
        if (this.editor.getValue().trim()) {
          NotificationManager.showInfo(
            `Document name changed to "${newTitle}". Auto-save enabled.`,
          );
        }
      }
    }
  ...
  showModal(title, body, buttons = []) {
    ...
    buttons.forEach((btn, index) => {
      const buttonElement = this.currentModalElement.querySelector(
        `[data-action="button-${index}"]`,
      );
      if (buttonElement) {
    ...
    const closeButton = this.currentModalElement.querySelector(
      '[data-action="close"]',
    );
    if (closeButton) {
    ...
    const backdrop = this.currentModalElement.querySelector(
