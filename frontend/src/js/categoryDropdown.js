...
export function initCategoryDropdown(documentUI) {
  ...
  function showNotification(msg, type) {
    ...
    } else if (NotificationManager && typeof NotificationManager.showNotification === 'function') {
  ...
  async function renderCategoryItems() {
    ...
    Array.from(categoryDropdownMenu.querySelectorAll('.category-item')).forEach(item => {
    ...
    try {
      ...
      console.debug('[CategoryDropdown] Loaded categories from localStorage:', categories);
    } catch (e) {
      categories = [];
      console.error('[CategoryDropdown] Error parsing categories from localStorage:', e);
    }
    ...
    if (generalIndex !== -1) {
      ...
      console.debug('[CategoryDropdown] Spliced out "General" for sorting:', general);
    }
    categories.sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
    if (general) categories.unshift(general);
    console.debug('[CategoryDropdown] Final categories for dropdown:', categories);
    // Find the divider <li> (declare once at top of function)
    const dividerLi = Array.from(categoryDropdownMenu.children)
      .find(li => li.querySelector && li.querySelector('hr.dropdown-divider'));
    ...
    categories.forEach(category => {
      ...
      if (category.trim().toLowerCase() !== 'general') {
        ...
        delBtn.className = 'btn btn-link btn-sm text-danger p-0 ms-2 delete-category-btn';
      ...
      a.addEventListener('click', function(e) {
        ...
        if (!docName.trim() || docName === 'Untitled Document') {
          ...
          showNotification('Please enter a valid document name before saving.', 'warning');
          return;
        }
        if (doc.category !== category) {
          ...
          if (documentUI && typeof documentUI.updateDocumentTitle === 'function') {
  ...
  document.getElementById('confirmDeleteCategoryBtn')?.addEventListener('click', async function() {
    ...
    const btn = document.querySelector(`.delete-category-btn[data-category="${categoryToDelete}"]`);
    ...
    if (ok) {
      showNotification(`Category '${categoryToDelete}' deleted.`, 'success');
      await renderCategoryItems();
    } else {
      showNotification(`Failed to delete category '${categoryToDelete}'.`, 'error');
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    console.debug('[CategoryDropdown] DOMContentLoaded event, calling renderCategoryItems');
    renderCategoryItems();
  });
  // Also call immediately in case DOM is already loaded
  if (document.readyState === 'interactive' || document.readyState === 'complete') {
    console.debug('[CategoryDropdown] Document already loaded, calling renderCategoryItems immediately');
    renderCategoryItems();
  }
  ...
  if (addCategoryForm && addCategoryInput && addCategoryBtn && categoryDropdownMenu) {
    ...
    addCategoryForm.addEventListener('submit', async function(e) {
      ...
      const exists = allCategories.some(cat => cat.trim().toLowerCase() === newCategory.toLowerCase());
      if (exists) {
      ...
      if (!added) {
        ...
        if (allCats.some(cat => cat.trim().toLowerCase() === newCategory.toLowerCase())) {
          showNotification('Category already exists.', 'warning');
        } else {
