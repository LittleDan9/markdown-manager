...
export class DocumentService {
  hasUnsavedChanges(currentContent, savedContent) {
    if (!savedContent) return currentContent.trim() !== "";
  ...
  deleteCategory(category) {
    ...
    if (this.isAuthenticated()) {
      // Backend: call API
      return fetch(
        "/api/v1/documents/categories/" + encodeURIComponent(category),
        {
          method: "DELETE",
        ...
        .then((res) => {
          ...
          return this.getAllCategories().then((cats) => {
  ...
  loadCurrentDocument() {
    try {
      ...
      const parsed = current
        ? JSON.parse(current)
        : { id: null, name: "Untitled Document", category: DEFAULT_CATEGORY };
  ...
  async saveDocument(content, name = null, id = null, category = null) {
    try {
      ...
      const docCategory =
        category || this.currentDocument.category || DEFAULT_CATEGORY;

      if (this.isAuthenticated()) {
        // Save to server for authenticated users
        const serverDocId =
          typeof id === "string" && id.startsWith("doc_") ? null : id;
      ...
      if (this.isAuthenticated()) {
        ...
        if (categoryFilter && categoryFilter !== "All") {
          documents = documents.filter(
            (doc) => (doc.category || DEFAULT_CATEGORY) === categoryFilter,
          );
        }
  ...
  createNewDocument(name = "Untitled Document", category = DEFAULT_CATEGORY) {
    ...
    this.saveCurrentDocument();
    // No editor.setValue here; UI should handle clearing editor
  ...
  async exportAsPDF(filename = null) {
    ...
    if (!previewElement.innerHTML.trim()) {
      throw new Error(
        "No content to export. Please add some content to the editor first.",
      );
    }
  ...
  async fetchDocumentsFromServer(category = null) {
    try {
      ...
      console.error(
        "fetchDocumentsFromServer: config.apiBaseUrl =",
        config.apiBaseUrl,
        ", window.location.origin =",
        window.location.origin,
      );
  ...
  async getCategoriesFromServer() {
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/documents/categories/`,
        {
          method: "GET",
          headers: this.getAuthHeaders(),
        },
    ...
    try {
      ...
      if (currentContent.trim() || this.currentDocument.id) {
        ...
        const currentCategory =
          this.currentDocument.category || DEFAULT_CATEGORY;
  ...
  async onUserLogout() {
    ...
    this.documents = this.loadDocuments();
    this.categories = this.loadCategories();
    // Reset current document to untitled/default
    this.currentDocument = {
      id: null,
      name: "Untitled Document",
      category: DEFAULT_CATEGORY,
    ...
    this.saveCurrentDocument();
    // Optionally, update UI fields if needed (depends on your UI framework)
    if (typeof this.updateDocumentUI === "function") {
      this.updateDocumentUI();
    }
    console.log(
      "User logged out, switched to localStorage mode, and reset current document.",
    );
  }
