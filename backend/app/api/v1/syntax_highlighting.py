...
async def highlight_code(request: HighlightRequest):
    ...
    try:
        ...
        is_supported = (
            ...
            if request.language else False
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error highlighting code: {str(e)}"
        )
...
async def get_language_info(language: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving language info: {str(e)}"
        )


@router.get("/languages/{language}/check")
async def check_language_support(language: str):
