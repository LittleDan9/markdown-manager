...
def upgrade() -> None:
    ...
    op.alter_column('categories', 'updated_at',
               ...
               existing_nullable=False,
               existing_server_default=sa.text('CURRENT_TIMESTAMP'))
    op.alter_column('custom_dictionaries', 'created_at',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               type_=sa.DateTime(),
               nullable=False,
               existing_server_default=sa.text('now()'))
    op.alter_column('custom_dictionaries', 'updated_at',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               type_=sa.DateTime(),
               nullable=False,
               existing_server_default=sa.text('now()'))
    ...
    if 'uq_user_dictionary_word' in index_names:
        op.drop_index(op.f('uq_user_dictionary_word'), table_name='custom_dictionaries', postgresql_where='(category_id IS NULL)')
    
    op.create_index(op.f('ix_custom_dictionaries_user_id'), 'custom_dictionaries', ['user_id'], unique=False)
    op.drop_constraint(op.f('custom_dictionaries_user_id_fkey'), 'custom_dictionaries', type_='foreignkey')
    op.create_foreign_key(None, 'custom_dictionaries', 'users', ['user_id'], ['id'], ondelete='CASCADE')
    ...
    op.add_column('documents', sa.Column('is_shared', sa.Boolean(), nullable=True, server_default=sa.text('FALSE')))
    
    # Update any existing NULL values to FALSE
    connection = op.get_bind()
    connection.execute(sa.text("UPDATE documents SET is_shared = FALSE WHERE is_shared IS NULL"))
    ...
    op.alter_column('users', 'mfa_enabled',
               ...
               nullable=False)
    op.alter_column('users', 'backup_codes',
               existing_type=postgresql.JSON(astext_type=sa.Text()),
               type_=sa.Text(),
               existing_nullable=True)
    op.alter_column('users', 'sync_preview_scroll_enabled',
               existing_type=sa.BOOLEAN(),
               nullable=False)
    op.alter_column('users', 'autosave_enabled',
               existing_type=sa.BOOLEAN(),
               nullable=False)
    ...
    op.alter_column('users', 'updated_at',
               ...
               nullable=False,
               existing_server_default=sa.text('now()'))
    op.create_foreign_key(None, 'users', 'documents', ['current_doc_id'], ['id'], ondelete='SET NULL')
...
def downgrade() -> None:
    ...
    op.create_foreign_key(op.f('custom_dictionaries_user_id_fkey'), 'custom_dictionaries', 'users', ['user_id'], ['id'])
    op.drop_index(op.f('ix_custom_dictionaries_user_id'), table_name='custom_dictionaries')
    ...
    if 'uq_user_dictionary_word' not in index_names:
        op.create_index(op.f('uq_user_dictionary_word'), 'custom_dictionaries', ['user_id', 'word'], unique=True, postgresql_where='(category_id IS NULL)')
    op.alter_column('custom_dictionaries', 'updated_at',
               existing_type=sa.DateTime(),
               type_=postgresql.TIMESTAMP(timezone=True),
               nullable=True,
