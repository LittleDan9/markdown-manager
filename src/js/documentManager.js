...
export class DocumentManager {
    ...
    /**
     * Export preview as PDF with proper emoji and formatting support
     */
    async exportAsPDF(filename = null) {
        try {
            ...
            try {
                // Capture the entire preview content as a high-quality image
                ...
                // Determine if we should use portrait or landscape
                const isLandscape = aspectRatio < 0.7;
                ...
                // If content is very long, split into multiple pages
                if (pdfHeight > pageHeight - (2 * margin)) {
                    ...
                    if (totalPages > 1) {
                        // Remove the first page and recreate with proper pagination
                        pdf.deletePage(1);

                        for (let page = 0; page < totalPages; page++) {
                            pdf.addPage();

                            const sourceY = page * pageContentHeight * (imgHeight / pdfHeight);
                            const sourceHeight = Math.min(pageContentHeight * (imgHeight / pdfHeight), imgHeight - sourceY);
                            ...
                            const pageHeight = sourceHeight * (pdfWidth / imgWidth);

                            pdf.addImage(pageImgData, 'PNG', xOffset, margin, pdfWidth, pageHeight);
                        }
                    }
                }
                ...
                console.warn('html2canvas failed, falling back to text export:', canvasError);
                return this.exportAsPDFTextFallback(filename);
            }
    ...
    /**
     * Fallback text-based PDF export (original implementation)
     */
    async exportAsPDFTextFallback(filename = null) {
        try {
            const previewElement = document.querySelector('#preview .preview-scroll');
            if (!previewElement) {
                throw new Error('Preview element not found');
            ...
            console.error('Error in text fallback PDF export:', error);
            throw new Error(`PDF export failed: ${error.message}`);
        }
    }
