...
# Extract DATABASE_URL from remote serve
...
# Check if backup script exists in backend directory
if [ ! -f "$WSL_BACKEND_DIR/../scripts/backup_production_data.py" ]; then
    echo -e "${RED}❌ Backup script not found in project${NC}"
    exit 1
fi
