"""
Main Icon API Router - Clean RESTful structure
...
from . import search, statistics, metadata, overview, legacy, batch

# Create main icon router
router = APIRouter(prefix="/icons", tags=["Icons API"])
...
# System overview and statistics (keep for backward compatibility)
...
# ============================================================================
# LEGACY ID-BASED ACCESS (Put last to avoid conflicts)
# ============================================================================

@router.get(
    "/{icon_id}",
    response_model=IconMetadataResponse,
    summary="[LEGACY] Get icon by ID",
    description="Legacy endpoint - prefer /packs/{pack}/contents/{key} instead",
...
async def get_icon_by_id(
    ...
    """[LEGACY] Get icon by ID - prefer RESTful endpoints instead."""
    try:
        ...
        # Add reference URLs pointing to preferred RESTful endpoints
        if icon.pack:
            pack_name = icon.pack.name
            icon.urls = {
                "recommended": f"/icons/packs/{pack_name}/contents/{icon.key}",
                "raw": f"/icons/packs/{pack_name}/contents/{icon.key}/raw",
                "svg": f"/icons/packs/{pack_name}/contents/{icon.key}/svg",
                "pack": f"/icons/packs/{pack_name}"
            }
