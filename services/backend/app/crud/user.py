...
async def update_user_password(
    ...
    if not verify_password(
        current_password, object.__getattribute__(db_user, "hashed_password")
    ):
        return False

    # Update password
    object.__setattr__(db_user, "hashed_password", get_password_hash(new_password))
    await db.commit()
    return True
...
async def setup_mfa(
    ...
    object.__setattr__(db_user, "totp_secret", secret)
    object.__setattr__(db_user, "backup_codes", backup_codes)
    object.__setattr__(db_user, "mfa_enabled", False)  # Not enabled until verified
    await db.commit()
    return True
...
async def enable_mfa(db: AsyncSession, user_id: int) -> bool:
    ...
    object.__setattr__(db_user, "mfa_enabled", True)
    await db.commit()
    return True
...
async def disable_mfa(db: AsyncSession, user_id: int) -> bool:
    ...
    object.__setattr__(db_user, "mfa_enabled", False)
    object.__setattr__(db_user, "totp_secret", None)
    object.__setattr__(db_user, "backup_codes", None)
    await db.commit()
    return True
