...
class AiPreferencesApi extends Api {
  ...
  async getPreferences() {
    const response = await this.apiCall('/api/ai/preferences', 'GET');
  ...
  async savePreferences({ preferred_provider, preferred_model, preferred_key_id }) {
    ...
    const response = await this.apiCall('/api/ai/preferences', 'PUT', body);
    return response.data;
  }

  /** Get current quota status (usage, limits, reset time). */
  async getQuota() {
    const response = await this.apiCall('/api/ai/usage/quota', 'GET');
    return response.data;
  }
}
