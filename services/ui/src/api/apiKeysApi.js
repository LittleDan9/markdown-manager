...
class ApiKeysApi extends Api {
  ...
  async getKeys() {
    const response = await this.apiCall('/platform-keys', 'GET');
  ...
  async addKey({ provider, api_key, label, base_url, preferred_model }) {
    ...
    const response = await this.apiCall('/platform-keys', 'POST', body);
    return response.data;
  }

  /** Update an existing key's configuration. */
  async updateKey(id, updates) {
    const response = await this.apiCall(`/platform-keys/${id}`, 'PUT', updates);
    return response.data;
  }

  /** Delete a key. */
  async deleteKey(id) {
    await this.apiCall(`/platform-keys/${id}`, 'DELETE');
  }

  /** Test a key by making a lightweight health-check call to the provider. */
  async testKey(id) {
    const response = await this.apiCall(`/platform-keys/${id}/test`, 'POST');
    return response.data;
  }

  /** Fetch available models from the provider using the stored API key. */
  async listModels(id) {
    const response = await this.apiCall(`/platform-keys/${id}/models`, 'POST');
    return response.data;
  }
}
